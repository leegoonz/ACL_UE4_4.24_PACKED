...
#include "Animation/AnimEncodingRegistry.h"	// @third party code - ACLPlugin
...
void AnimationFormat_SetInterfaceLinks(CompressedDataType& CompressedData)
{
	...
	{
		// @third party code - BEGIN ACLPlugin
		const bool bSuccess = FAnimEncodingRegistry::Get().SetInterfaceLinks(CompressedData.KeyEncodingFormat, CompressedData.RotationCodec, CompressedData.TranslationCodec, CompressedData.ScaleCodec);
		if (!bSuccess)
		{
			UE_LOG(LogAnimationCompression, Fatal, TEXT("%i: unknown or unsupported animation format"), (int32)CompressedData.KeyEncodingFormat );
		}
